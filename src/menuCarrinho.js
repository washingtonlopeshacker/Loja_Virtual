...
function desenharProdutoNoCarrinho(idProduto) {
  ...
  const cartaoProdutoCarrinho = `<button id="remover-item-${
    ...
    <div class="p-2 flex flex-col justify-between">
      ...
      <p class="text-slate-400 text-xs">Computador: </p>
      <p class="text-green-700 text-lg">$${produto.preco}</p>
    </div>
    <div class='flex text-slate-950 items-end absolute bottom-0 right-2 text-lg'>
        <button id='decrementar-produto-${produto.id}'><i class="fa-solid fa-cart-arrow-down fa-bounce"></i></button>
        <p id='quantidade-${produto.id}' class='ml-2'>${
    idsProdutoCarrinhoComQuantidade[produto.id]
  }</p>
        <button class='ml-2' id='incrementar-produto-${produto.id}'><i class="fa-solid fa-cart-plus fa-bounce"></i></button>
    </div>`;
...
export function atualizarPrecoCarrinho() {
  ...
  salvarLocalStorage("precoTotalCarrinho", precoTotalCarrinho)
